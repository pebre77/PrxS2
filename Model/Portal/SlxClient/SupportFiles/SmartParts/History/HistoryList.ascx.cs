...
public partial class SmartParts_History_HistoryList : EntityBoundSmartPartInfoProvider
{
    ...
    protected override void OnFormBound()
    {
		...
		//Praxi Solutions,Inc
		//Enable Database Change in History
		//10/13/2011 - FHV
		if (vEntity){ //If is Opportunity Show DatabaseChange
		HqlBindingSource.Where = string.Format("h.UserId = ui.id and h.Type != {0} and  h.{1} = '{2}'", (int)HistoryType.atNote,keyId, entityID);		
		} else {
    ...
    protected static string GetImage(object activityType)
    {
        ...
        switch (activityType.ToString())
        {
			...
			case "Database Change":
				return "images/icons/Options_16x16.gif";
            default:
				return meetingURL;
        }
    }
